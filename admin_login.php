   ...
   print_html_menu();
    ...
    print_div_open("middle_container_id","middle_container_full");
     ...
     print_div_open("fullmiddle_col_id","fullmiddle_col");
      print_clearline(2);
      if (isset($_POST['ides'])) $IDES=validate_unsigned($_POST['ides']); else $IDES=0;
      update_servers_ides($IDES);
      $epp_prot=get_eppserver_proto($IDES);

      $EPP_CONNECTED=eppOpenConnection($eppsock);

      if (!$EPP_CONNECTED) {
       echo "Connessione fallita!<br>";
       die();
      }
      echo "Account Registrar: <b>$epp_username_A</b> <br>";
      echo "$LANGUAGE[399] <b>$eppsock</b> <br>";
      echo "$LANGUAGE[400] <b>$IDES</b> <br>";  
      echo "$LANGUAGE[401] <b>$epp_prot</b> <br>";  

      if (($epp_prot=="TCP")&&($TCP_OPEN_HANDSHAKE)) {
       $xml=eppTcpGetFrame($eppsock)."\n\n";
       echo "
        Risposta Server a Connessione TCP: <br>
        <textarea cols=64 rows=8>$xml</textarea><br>
        <pre>$xml</pre>
        <br>
       ";
      }  
      ...
      if ($epp_prot=="TCP") {
       echo "<b>$LANGUAGE[402]</b><br>";
       $xml=eppReadXMLScheme("hello");
       echo "Avvenuta lettura Schema XML per HELLO<br>";
       echo "<textarea>$xml</textarea><br>";
       eppTcpSendCommand($eppsock,$xml);
       echo "Avvenuto Invio Comando HELLO<br>";
       $xml=eppTcpGetFrame($eppsock)."\n\n";
       echo "
        Risposta Server: <br>
        ...
        <br>
       ";
       test_client_debug_info($xml,$RESCODE,$REASONCODE);
      } 

      if ($IDES>0) $xml=epp_login_db_account($eppsock,$ID_ADMIN);
       else $xml=epp_login_ext($eppsock, "$epp_username_A", "$epp_password_A");
      ...
      if (($epp_prot=="TCP")||($epp_prot=="TCP2")) {
       $SESSIONID=$eppsock;
       echo "$LANGUAGE[399] <b>$SESSIONID</b> <br><br>"; 
      } else {
