 ...
 ConnectDB();
  ...
  print_html_top("$LANGUAGE[374]");
   print_html_menu();
    ...
    $ID_LEVEL=get_userlevel_noredir($ID_UTENTE);  
    ...
    print_div_open("middle_container_id","middle_container_full");
     ...
     print_div_open("fullmiddle_col_id","fullmiddle_col");
      ...
      print_clearline(1);
      if ($ID_LEVEL<=1) {
       form_cambia_utente_e_password();
       print_clearline(1);
      }
      if ($ID_LEVEL<=1) {
       form_admin_modifica_utente($ID_UTENTE);
       print_clearline(1);
      }
      if ($ID_LEVEL<=1) {
       form_admin_aggiungi_utente($ID_UTENTE);
       print_clearline(1);
      }
      if ($ID_LEVEL<=1) {
       lista_utenti_admin();
