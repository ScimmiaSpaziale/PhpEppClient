<?
 ...
 function webpanel_domain_list_expiring_clear($PAG){
  ...
  $START=($PAG-1)*$N;
  DBSelect("
   SELECT * FROM domain_names WHERE (expire>=$today) AND (status=1) AND (idregistrant='FINERVKINGDANID') ORDER BY expire ASC LIMIT $START,$N
  ",$rs);
  #DBSelect("
  # SELECT * FROM domain_names WHERE (status=1) AND (idregistrant='FINERVKINGDANID') ORDER BY expire ASC LIMIT $START,$N
  #",$rs);
  $C=0;
 ...
 if (isset($_GET['pag'])) $PAG=validate_unsigned($_GET['pag']); else $PAG=0;
