<?
 ...
 ConnectDB();
  ...
  $STATUS=get_contact_status($ID); 

  if ($STATUS=="Pending") {
   webpanel_delete_contact($IDC);
  } else if (($STATUS=="Active")||(($STATUS=="AdminActive"))) {
   ...
   $xml=epp_contact_delete($eppsock, $CID);
   $RESCODE=xml_get_resultcode($xml);
   $REASONCODE=xml_get_reasoncode($xml);
   eppCloseConnection($eppsock);

   if ($RESCODE==2303) { update_contact_status($ID,"Pending"); }
   if ($RESCODE==1000) { update_contact_status($ID,"Pending"); }
