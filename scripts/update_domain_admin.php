<?
 ...
 ConnectDB();
  ...
  $IDNEWADM=validate_string($_POST['newadmin']);
  $IDOLDADM=$IDADM;
  ...
  if ($RESCODE==1000) {
   echo "$LANGUAGE[495] <b>$DOMAIN</b> <br><br>";
   webpanel_logs_add("EppProto","$LANGUAGE[495] $DOMAIN.");
   update_domain_updating($IDD);
   update_domain($IDD, $DOMAIN, $IDREG, $IDNEWADM, $IDTECH, $IDBILL);
  } else {
   echo "$LANGUAGE[496] <b>$DOMAIN</b> <br><br>";
   webpanel_logs_add("EppProto","$LANGUAGE[496] $DOMAIN, $LANGUAGE[250] $RESCODE, $LANGUAGE[251] $REASONCODE.");
  }
