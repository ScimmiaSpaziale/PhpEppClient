<?
 ...
 ConnectDB();
  ...
  $xml=epp_update_pubblishing_consent($eppsock, $CID, $PUBFLAG);
  $RESCODE=xml_get_resultcode($xml);
  $REASONCODE=xml_get_reasoncode($xml);
  eppCloseConnection($eppsock);

  if ($RESCODE==1000) {
   echo "$LANGUAGE[489] <b>$CID</b> <br><br>";
   webpanel_logs_add("EppProto","$LANGUAGE[489] $CID.");
   update_contact_privacy($ID,$PUB);
  } else {
   webpanel_logs_add("EppProto","$LANGUAGE[490] $CID, $LANGUAGE[250] $RESCODE, $LANGUAGE[251] $REASONCODE.");
  }
