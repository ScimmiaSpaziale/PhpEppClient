<?
 ...
 ConnectDB();
  ...
  if ($RESCODE!=1000) {
   ...
   webpanel_logs_add("EppProto","$LANGUAGE[429] $DOMAIN.");
  } else {
   echo "$LANGUAGE[380] <br>";
   webpanel_logs_add("EppProto","$LANGUAGE[430] $DOMAIN.");
   $AVAILABLE=xml_domain_availability($xml);
   echo "$LANGUAGE[432] $AVAILABLE<br>";
   if ($AVAILABLE=="true") {
    echo "$LANGUAGE[433] <b>$DOMAIN</b> $LANGUAGE[434] <br>";
   } else {
    echo "$LANGUAGE[433] <b>$DOMAIN</b> $LANGUAGE[435] <br>";
    echo "$LANGUAGE[436] ".xml_domain_no_availability_reason($xml)." <br>";
   }
  }
