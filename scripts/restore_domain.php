<?
 ...
 ConnectDB();
  ...
  $ID_LEVEL=get_userlevel($ID_UTENTE);  
  ...
  echo "<br> $LANGUAGE[456] [$ID] <b>$DOMAIN</b> <br><br>";
  ...
  if (($ID_LEVEL<=1) || ($AVAILABLE_CRD>$R_CRD)) {
  ...
  if ($RESCODE==11111) {
   echo "Comando non eseguito per insufficienza di credito. <br>";
   print_epp_error($RESCODE, $REASONCODE);
   webpanel_logs_add("EppProto","$LANGUAGE[457] $DOMAIN.");
  } else if ($RESCODE!=1000) {
   print_epp_error($RESCODE, $REASONCODE);
   webpanel_logs_add("EppProto","$LANGUAGE[457] $DOMAIN.");
  } else {
   echo "Comando eseguito con successo! <br>";
   webpanel_logs_add("EppProto","$LANGUAGE[458] $DOMAIN.");
   update_domain_status($ID,1);
   update_domain_updating($ID);
   if ($ID_LEVEL>1) {
    log_used_crd($ID_USER, $R_CRD, "Dominio ripristinato: $DOMAIN");
    dec_credits($ID_USER, $R_CRD);
   }
  }
