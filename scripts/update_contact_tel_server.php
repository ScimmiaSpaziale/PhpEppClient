<?
 ...
 ConnectDB();
  ...
  $xml=epp_contact_update_telfax($eppsock, $CID, $TEL, $FAX);
  $RESCODE=xml_get_resultcode($xml);
  $REASONCODE=xml_get_reasoncode($xml);
  eppCloseConnection($eppsock);

  if ($RESCODE==1000) {
   echo "$LANGUAGE[491] <b>$CID</b> <br><br>";
   webpanel_logs_add("EppProto","$LANGUAGE[491] $CID.");
   update_contact_tel($ID,$TEL);
   update_contact_fax($ID,$FAX);
  } else {
   webpanel_logs_add("EppProto","$LANGUAGE[492] $CID, $LANGUAGE[250] $RESCODE, $LANGUAGE[251] $REASONCODE.");
  }
