<?
 ...
 ConnectDB();
  ...
  $xml=epp_change_admin_registrant($eppsock, $DOMAIN, $IDREG, $IDADM, $EPPCODE);
  $RESCODE=xml_get_resultcode($xml);
  $REASONCODE=xml_get_reasoncode($xml);
  eppCloseConnection($eppsock);

  if ($RESCODE==1000) {
   echo "$LANGUAGE[497] <b>$DOMAIN</b> <br><br>";
   webpanel_logs_add("EppProto","$LANGUAGE[497] $DOMAIN.");
   update_domain_updating($IDD);
  } else {
   echo "$LANGUAGE[498] <b>$DOMAIN</b> <br><br>";
   webpanel_logs_add("EppProto","$LANGUAGE[498] $DOMAIN, $LANGUAGE[250] $RESCODE, $LANGUAGE[251] $REASONCODE.");
  }
