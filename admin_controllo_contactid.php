   ...
   print_html_menu();
    ...
    print_div_open("middle_container_id","middle_container_full");
     ...
     print_div_open("fullmiddle_col_id","fullmiddle_col");
      ...
      $ID=validate_string($_GET[contactid]);
      echo "<br> $LANGUAGE[379] <b>$ID</b> <br><br>";
      ...
      $xml=epp_contact_check($eppsock,$ID);
      $RESCODE=xml_get_resultcode($xml);
      $REASONCODE=xml_get_reasoncode($xml);
      webpanel_logs_add("EppProto","$LANGUAGE[379] $ID");
      if ($RESCODE!=1000) {
       print_epp_error($RESCODE,$REASONCODE);
       webpanel_logs_add("EppProto","$LANGUAGE[387] $RESCODE,$REASONCODE");
       eppCloseConnection($eppsock);
      } else {
       echo "$LANGUAGE[380] <br>";
       $AVAILABLE=xml_contact_availability($xml);
       if ($AVAILABLE=="true") {
        echo "$LANGUAGE[381] <b>$ID</b> $LANGUAGE[382] <br>";
        webpanel_logs_add("EppProto","$LANGUAGE[381] <b>$ID</b> $LANGUAGE[382]");
       } else {
        echo "$LANGUAGE[381] <b>$ID</b> $LANGUAGE[383] <br>";
        webpanel_logs_add("EppProto","$LANGUAGE[381] <b>$ID</b> $LANGUAGE[383]");
       } 
      }

      print_debug_info($xml,"contactcheck",80,12,$RESCODE,$REASONCODE,$ID_EPPSESSION);
