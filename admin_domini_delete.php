 ...
 ConnectDB();
  print_html_top("$LANGUAGE[374]");
   print_html_menu();
    ...
    if (isset($_GET['id'])) $IDDOMAIN=validate_unsigned($_GET['id']); else $IDDOMAIN=0;
    $DOMAIN_STATUS=get_domain_status($IDDOMAIN);     
    ...
    print_div_open("middle_container_id","middle_container_full");
     ...
     print_div_open("fullmiddle_col_id","fullmiddle_col");
      ...
      print_clearline(2);
       ...
       if ($DOMAIN_STATUS==1) form_delete_domain($ID_ADMIN, $IDDOMAIN);
        else form_remove_domain($ID_ADMIN, $IDDOMAIN);

       if ($DOMAIN_STATUS==1) form_delete_domain_on_expire_starting($ID_ADMIN, $IDDOMAIN);
        
       if ($DOMAIN_STATUS==1) form_delete_domain_on_expire_ending($ID_ADMIN, $IDDOMAIN);
